			...
			if( line[0] == '+' or line[0] == '-' ) {
				...
				if( space == string::npos )
					...
					if( token == _namespace_ ) {
						...
						if( bind ) {
							auto namespace_and_include =
								split_in_two(name, "Invalid line for include_for_namespace specification! Must be: name_of_type + <space or tab> + include_path. Got: " + line);
						...
						if( bind ) {
							auto binder_function =
								split_in_two(name, "Invalid line for add_on_binder_for_namespace specification! Must be: name_of_type + <space or tab> + name_of_binder. Got: " + line);
						...
						throw std::runtime_error("Invalid token in config file! Each token must be ether: namespace, class or function! For example: '+function aaa::bb::my_function'. Token: '" +
												 token + "' Line: '" + line + '\'');
				}
			}
